...
public class FollowCircuit : MonoBehaviour {
    ...
    // Store a tracker that the players will follow
    GameObject tracker;
    ...
    void ProcessTracker() {
        ...
        // Check the tracker doesn't get to far ahead of the player
        if (Vector3.Distance(tracker.transform.position, this.transform.position) > lookAhead) return;

        // Check if the player has reached a certain distance from the current waypoint
